...
var riot=require("riot"),fbRef=require("../../firebase"),utils=require("../../utils"),moment=require("moment");module.exports=function(){function e(e,a){if("project"==e&&a){if(o.trigger("project_clear"),r==a)return void o.trigger("project_data",i);r=a,fbRef.child("project/"+r).on("value",t)}}function t(e){var t={pid:r,isOwner:fbRef.getAuth().uid==e.val().userId};i=utils.extend(e.val(),t);var a="YYYY-MM-DD",n=moment.unix(i.date.start),u=moment.unix(i.date.end);i.dateStart=n.format(a),i.dateEnd=u.format(a),o.trigger("project_data",i)}riot.observable(this);var r,i,o=this;o.on("route",e)};
},{"../../firebase":5,"../../utils":44,"moment":48,"riot":55}],18:[function(require,module,exports){
var riot=require("riot"),fbRef=require("../../../firebase");module.exports=function(){function r(r,a,f){if("project"==r||a||f){if(o.trigger("ringform_reset"),i==a)return void o.trigger("ringers_data",n);t=fbRef.child("ringer/"+a).orderByChild("active").equalTo(!0),t.on("value",e),i=a}}function e(r){n.length=0,r.forEach(function(r){n.push({uid:r.key(),sign:r.val().sign})}),o.trigger("ringers_data",n)}riot.observable(this);var i,t,o=this,n=[];o.on("route",r)};
...
},{"../../../firebase":5,"riot":55,"xhr":57}],21:[function(require,module,exports){
...
},{"../../firebase":5,"riot":55}],22:[function(require,module,exports){
...
var riot=require("riot");riot.tag("dropdown",'<label>{ capitalize(opts.label) }</label><br> <select id="select" name="{ opts.label }" __required="{ opts.require }"></select> <div id="options" style="display:none;"> <option value="">Select…</option> <yield></yield> </div>',function(){require("riotcontrol");this.capitalize=function(e){return e[0].toUpperCase()+e.substr(1)}.bind(this),this.select.innerHTML=this.options.innerHTML});
},{"riot":55,"riotcontrol":56}],27:[function(require,module,exports){
...
var riot=require("riot");riot.tag("ringform",'<h2>{ opts.action == \'newring\' ? \'New Ring\' : \'Control\' }</h2> <form name="frmRing" onsubmit="{ saveRing }"> <div riot-tag="ringid" if="{ opts.action == \'control\' }"></div> <div riot-tag="species"></div> <div if="{ opts.action == \'newring\' }"> <div> <label>Ring #</label><br> <input riot-type={"number"} name="ringNumber" step="1" min="0" max="99" required> </div> <input type="hidden" name="snid"> <div riot-tag="dropdown" label="ringSize" require="required"> <option each="{ ringsize }" value="{ size }">{ size }</option> </div> </div> <div> <label>Net</label><br> <input type="text" name="net"> </div> <div riot-tag="dropdown" label="age"> <option each="{ val in parent.age }" value="{ val }">{ val }</option> </div> <div riot-tag="dropdown" label="sex"> <option each="{ val in parent.sex }" value="{ val }">{ val }</option> </div> <div> <label>PJM</label><br> <input riot-type={"number"} step="1" min="0" max="9" name="pjm"> </div> <div> <label>Fat</label><br> <input riot-type={"number"} step="1" min="0" max="9" name="fat"> </div> <div riot-tag="dropdown" label="sign" require="required"> <option each="{ parent.ringers }" value="{ sign }">{ sign }</option> </div> <div> <label>Weight</label><br> <input type="text" name="weight" autocomplete="off" onblur="{ checkMinMax }"> <p if="{ minMax.weight.hint }">Average weight of this species is { minMax.weight.min }&ndash;{ minMax.weight.max } g.</p> </div> <div> <label>Wing length</label><br> <input type="text" name="wing" autocomplete="off" onblur="{ checkMinMax }"> <p if="{ minMax.wing.hint }">Average winglength of this species is { minMax.wing.min }&ndash;{ minMax.wing.max } mm.</p> </div> <div> <label>Primaries</label><br> <input type="text" name="primaries" autocomplete="off"> </div> <div> <label>Secondaries</label><br> <input type="text" name="secondaries" autocomplete="off"> </div> <div> <label>Comment</label><br> <textarea name="comment" autocomplete="off"></textarea> </div> <div> <button type="submit">Save</button> <button type="button" onclick="{ resetForm }">Reset</button> </div> </form>',function(i){var e=require("riotcontrol"),n=require("form-serialize"),t=this;t.sex=["F","M"],t.age=["1.0","2.0","2+","3+"],this.checkMinMax=function(i){var e=i.target.name,n=i.target.value;if(n){var r=t.minMax[e].min,a=t.minMax[e].max;(r||a)&&(n<t.minMax[e].min||n>t.minMax[e].max)&&(t.minMax[e].hint=!0)}}.bind(this),this.setSpeciesData=function(i){t.minMax={weight:{min:i.MinVikt,max:i.MaxVikt,hint:!1},wing:{min:i.MinVinge,max:i.MaxVinge,hint:!1}},t.setRingsize(i.Rtyp1.replace(",","."))}.bind(this),this.setRingsize=function(n){if("control"!=i.action){t.frmRing.ringSize.value=n;var r=t.frmRing.ringSize.selectedIndex;if(-1==r){var a="Suggested ring size ("+n+") not listed in Ring Sizes.";return void e.trigger("alert",a,"warning")}t.setSnid(n)}}.bind(this),this.setSnid=function(i){t.ringsize.forEach(function(e){e.size==i&&(t.snid.value=e.snid)})}.bind(this),this.saveRing=function(){var r=n(t.frmRing,{hash:!0});if("newring"==i.action){if(!r.ringSize)return void e.trigger("alert","Ring size missing. Unable to create unique ring id.","error");r.ringNumber=parseInt(r.ringNumber)<10?"0"+r.ringNumber:r.ringNumber,r.id=r.snid?r.snid+r.ringNumber:r.ringNumber}r.species=r.species.toUpperCase(),delete r.ringNumber,delete r.ringSize,delete r.snid,console.log("save",i.action,"data",r)}.bind(this),this.resetForm=function(){t.frmRing.reset()}.bind(this),e.on("ringers_data",function(i){t.update({ringers:i})}),e.on("ringsize_data",function(i){t.update({ringsize:i})}),e.on("ringform_reset",t.resetForm)});
