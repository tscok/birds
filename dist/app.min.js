...
var riot=require("riot"),fbRef=require("../../firebase");module.exports=function(){function i(i){o=i,s=fbRef.child("ringlist/"+o),s.orderByChild("size").on("value",r)}function r(i){g.length=0,i.forEach(function(i){g.push({rsid:i.key(),size:i.val().size,snid:i.val().snid})}),d.trigger("ringlist_data",g)}function e(i){s.push(i,l)}function t(i){s.child(i.rsid).remove(l)}function n(r,e,t){return"project"==r&&e&&!t?o&&o==e&&g.length?(d.trigger("ringlist_hide"),void d.trigger("ringlist_data",g)):void i(e):void 0}riot.observable(this);var o,s,d=this,g=[],l=function(i){i&&d.trigger("alert",i.message,"error")};d.on("route",n),d.on("ringlist_add",e),d.on("ringlist_remove",t)};
...
var riot=require("riot");riot.tag("button-input",'<label>{ label }</label><br> <button type="button" each="{ buttons }" onclick="{ parent.select }" value="{ value }" class="{selected: selected }">{ label }</button> <input type="hidden" name="{ label }" value="hepp">',".selected { color: red; }",function(e){this.buttons=this.opts.options,this.label=e.label,this.select=function(e){var t=e.item.selected;this.buttons.forEach(function(e){e.selected&&(e.selected=!1)}),e.item.selected=t?!1:!0;var l=this[this.label],i=l.value,s=e.item.value;l.value=i!=s?s:""}.bind(this)});
},{"riot":58}],26:[function(require,module,exports){
